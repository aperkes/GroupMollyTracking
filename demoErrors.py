...
from processTracks import get_stats,get_meta,get_tracks,get_distance,clean_track,get_velocity_array
...
## Calculate lots of stat arrays for plotting
    velocity_array = get_velocity_array(track_array,None)
    flat_vel = np.reshape(velocity_array,[-1,1])
    n_zero = np.sum(flat_vel == 0)
    n_high = np.sum(flat_vel > 500)
    info_str = "N zeros: " + str(n_zero) + ". N > 500: " + str(n_high)
    ...
    ax1.scatter(flat_tracks[:,0],flat_tracks[:,1],alpha=0.01)
    ax1.set_xlabel('x position')
    ax1.set_ylabel('y position')

    ax.set_ylabel('x position (binned)')
    ax.set_xlabel('y position (binned)')
    
    fig2,axes = plt.subplots()
    axes.hist(flat_vel,bins=50)
    axes.set_title(info_str)
    axes.set_xlabel('Velocity (pixel/sec)')
