...
if __name__ == "__main__":
    ...
    for csv_file in os.listdir('./groupCSVs'):
        TEXT = False   
        #csv_file = sys.argv[1]
        if len(sys.argv) > 2:
            vid_file = sys.argv[2]
...
## Calculate lots of stat arrays for plotting
        ...
        if vid_file is not None:
            ...
            alpha = None
        ax.imshow(foo[0],vmin=0,vmax=100,alpha=alpha,cmap='viridis',extent=[0,800,0,800])

        fig.savefig('./imgs/' + csv_file.replace('.csv','png'),dpi=300)
        #plt.show()
